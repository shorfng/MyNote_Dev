> 当前位置：【Java】05_Database（数据库）-> 5.4_Oracle
...
mv /usr/local/upload/database /usr/local/Oracle11g/
...
passwd oracle
#此处需要设置密码oracle123456

# 查看 oracle id
# 结果为 uid=1000(oracle) gid=1000(oinstall) 组=1000(oinstall),1001(dba)
id oracle
```
...
- 将两个压缩包解压到同一个文件夹中
- 在解压文件夹database中找到可执行文件【setup.exe】双击安装。
